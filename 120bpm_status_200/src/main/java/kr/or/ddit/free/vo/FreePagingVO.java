...
/**
 * 페이징 처리 관련 속성 집합
 * 페이징처리는 전체 데이터를 대상으로 구간별로 쪼개어 조회하는 구조.
 * 페이징 속성 - 한 구간안에 몇개의 데이터를 포함시킬것인가, 한 화면안에 몇개의 구간에 대한 정보를 제공할 것인가
 * 
 *  1. 토탈레코드 - 100 (DB로 조회)
 *  2. 스크린사이즈 - 10(데이터건수(스타트로우(첫번째건수), 엔드로우) //임의로 정해
 *  3. 토탈페이지 - 10
 *  4. 블록사이즈 - 한페이지 안에서 다른링크로 이동할수 있는 페이지 제공하는 것. << 6,7,8,9,10 >>
 *  
...
public class FreePagingVO<T> { //최초만들어질때 pagingVO-> Controller ->memberdao시그니처 처리위해 2개의 메소드(totalRecord, 구간별조회레코드 메소드) ->마바 
//타입변수T로 객체가 선언X 생성O될 때 element타입 결정
//totalRecord - ((currentPage-1) * screenSize + endRow )
...
public FreePagingVO(int screenSize, int blockSize) { //필요하다면 두개의 생성자를 이용해 값을 바꿀 수 있다.
...
private FreeBoardVO freeInfo;

public void setFreeInfo(FreeBoardVO freeInfo) {
...
public void setPrjctNo(int pCode) {
